...
from models import setup_db, Blog, db_drop_and_create_all

Access_token_blogger = os.environ.get('BLOGGER')
Access_token_Visitor = os.environ.get('VISITOR')
...
class ThoughtsTestCase(unittest.TestCase):
    ...
    def test_create_new_blog(self):
        res = self.client().post(
            ...
            headers={
                'Authorization': 'Bearer ' + Access_token_blogger})
        data = json.loads(res.data)
    ...
    def test_405_if_blogs_creation_not_allowed(self):
        res = self.client().post(
            ...
            json={
                ...
                'content': 'This is a new blog'},
            headers={
                'Authorization': 'Bearer ' + Access_token_Visitor})
        data = json.loads(res.data)
...
# PATCH  '${blogger_id}/blogs/${blog_id}'
    ...
    def test_delete_blog(self):
        res = self.client().delete('/1/blogs/1',
                                   headers={'Authorization': 'Bearer '
                                            + Access_token_blogger})
        data = json.loads(res.data)
